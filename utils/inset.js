function inset(database) {
  const sheng = ["河北省", "山西省", "辽宁省", "吉林省", 
  "江苏省", "浙江省", "安徽省", "福建省", "江西省", "山东省", "河南省", 
  "湖北省", "湖南省", "广东省", "海南省", "四川省", "贵州省", "云南省",
   "陕西省", "甘肃省", "青海省", "台湾省"]
   const xing = ["赵","钱","孙","李","周","吴","郑","王","冯","陈","褚","卫","蒋","沈","韩","杨","朱","秦","尤",
...
for (let i = 0; i < 500; i++) {
  ...
  let j1 = Math.round(Math.random()*100);
  let k1 = Math.round(Math.random()*100);
  let a = Math.round(Math.random()*30);
  let z = Math.round(Math.random()*22);
  let p = Math.round(Math.random()*5);
  database.create({
    inspectionNum: `11312018000${i}`,
